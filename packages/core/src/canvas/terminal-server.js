...
export function setupTerminalServer(httpServer, base = '/', branch = 'unknown') {
  ...
  httpServer.on('upgrade', (req, socket, head) => {
    ...
    const widgetAgentId = params.get('agentId') || null
    const widgetWidth = params.get('width') ? Number(params.get('width')) : null
    const widgetHeight = params.get('height') ? Number(params.get('height')) : null

    wss.handleUpgrade(req, socket, head, (ws) => {
      handleConnection(ws, sessionId, canvasId, prettyName, widgetStartupCommand, { agentId: widgetAgentId, width: widgetWidth, height: widgetHeight })
    })
  })
}

/**
 * Poll tmux pane until the shell prompt appears, then run the full
 * startup sequence: env exports → clear → startup command / welcome screen.
 *
 * Replaces the old fixed-timeout approach which raced between the shell
 * prompt, env exports, clear, and the startup command.
 */
async function runNewSessionStartup({ tmuxName, ptyProcess, ws, startupCommand, widgetId, canvasId, branch, serverUrl, prettyName, termCfg }) {
  // Step 1: Wait for shell prompt (poll tmux pane for a prompt character)
  const promptReady = await waitForShellPrompt(tmuxName, 5000)
  if (!promptReady) {
    console.warn(`[storyboard] shell prompt not detected for ${tmuxName}, proceeding anyway`)
  }

  // Step 2: Export identity env vars + shell-config overrides via send-keys
  const envExports = [
  ...
  // Step 3: Wait for exports to complete, then clear
  await waitForShellPrompt(tmuxName, 3000)
  try {
    execSync(`tmux send-keys -t "${tmuxName}" -l "clear"`, { stdio: 'ignore' })
    execSync(`tmux send-keys -t "${tmuxName}" Enter`, { stdio: 'ignore' })
  } catch {}

  // Step 4: Wait for clear to finish, then run startup command or welcome
  await waitForShellPrompt(tmuxName, 2000)

  // Build the welcome-screen command (reused for both paths)
  const canvasArg = canvasId !== 'unknown' ? canvasId : ''
  const nameArg = prettyName ? ` --name "${prettyName}"` : ''
  const welcomeCmd = `storyboard terminal-welcome --branch "${branch}" --canvas "${canvasArg}"${nameArg}`

  if (startupCommand && startupCommand !== 'shell') {
    // Run the startup command, then fall back to the welcome screen on exit
    const wrappedCmd = `${startupCommand}; exec ${welcomeCmd}`
    try {
      execSync(`tmux send-keys -t "${tmuxName}" -l ${JSON.stringify(wrappedCmd)}`, { stdio: 'ignore' })
      execSync(`tmux send-keys -t "${tmuxName}" Enter`, { stdio: 'ignore' })
    } catch {}

    // For copilot-like agents, poll for readiness and send /allow-all
    if (startupCommand === 'copilot' || startupCommand === 'copilot --agent terminal-agent') {
      pollForCopilotReady(tmuxName)
    }
  } else if (!startupCommand) {
    // No startupCommand — show the welcome screen directly
    try {
      execSync(`tmux send-keys -t "${tmuxName}" -l ${JSON.stringify(welcomeCmd)}`, { stdio: 'ignore' })
      execSync(`tmux send-keys -t "${tmuxName}" Enter`, { stdio: 'ignore' })
    } catch {}
  }
  // startupCommand === 'shell' — nothing to do, shell is already running

  // Execute startup sequence if configured
...
/**
 * Poll tmux pane until a shell prompt character appears.
 * Returns true if prompt detected, false on timeout.
 */
async function waitForShellPrompt(tmuxName, timeout = 5000) {
  const start = Date.now()
  while (Date.now() - start < timeout) {
    try {
      const content = execSync(`tmux capture-pane -t "${tmuxName}" -p`, {
        encoding: 'utf8',
        timeout: 1000,
      })
      // Look for common prompt patterns: $, %, >, ❯, or the storyboard prompt
 ...
 */
function pollForCopilotReady(tmuxName) {
  let allowSent = false
...
function handleConnection(ws, widgetId, canvasId, prettyName, widgetStartupCommand = null, extraProps = {}) {
  ...
  // Build widgetProps from all available params (merge with existing to avoid losing data)
  const widgetProps = {}
  if (prettyName) widgetProps.prettyName = prettyName
  if (widgetStartupCommand) widgetProps.startupCommand = widgetStartupCommand
  if (extraProps.agentId) widgetProps.agentId = extraProps.agentId
  if (extraProps.width) widgetProps.width = extraProps.width
  if (extraProps.height) widgetProps.height = extraProps.height

  // Write terminal config for agent context
  writeTermConfig({ branch, canvasId, widgetId, serverUrl, tmuxName, displayName: prettyName || null, widgetProps: Object.keys(widgetProps).length > 0 ? widgetProps : null })
  ...
  if (hasTmux) {
    ...
    // For new sessions, run the startup sequence: wait for shell → env exports → clear → startup command.
    // Everything is sequential to avoid race conditions between the shell prompt,
    // env exports, clear, and the startup command.
    if (isNewSession) {
      const startupCommand = widgetStartupCommand ?? termCfg.startupCommand ?? null

      runNewSessionStartup({
        tmuxName, ptyProcess, ws, startupCommand,
        widgetId, canvasId, branch, serverUrl, prettyName, termCfg,
      })
    }
