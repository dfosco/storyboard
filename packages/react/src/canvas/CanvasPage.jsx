...
import { findConnectedSplitTarget, readSplitFromURL, clearSplitFromURL } from './widgets/expandUtils.js'
...
export default function CanvasPage({ canvasId: canvasIdProp, name, siblingPages = [], canvasMeta = null }) {
  ...
  // Restore split-screen from `?split=primaryId,secondaryId` URL param.
  // Dispatches a custom event that expandable widgets listen for.
  const splitRestoredRef = useRef(false)
  useEffect(() => {
    if (loading || splitRestoredRef.current) return
    const split = readSplitFromURL()
    if (!split) return
    // Verify both widgets exist in the current canvas
    const widgets = localWidgets ?? []
    const primaryExists = widgets.some((w) => w.id === split.primaryId)
    const secondaryExists = widgets.some((w) => w.id === split.secondaryId)
    if (!primaryExists || !secondaryExists) {
      clearSplitFromURL()
      return
    }
    splitRestoredRef.current = true
    // Give widgets a frame to mount, then dispatch the restore event
    requestAnimationFrame(() => {
      document.dispatchEvent(new CustomEvent('storyboard:restore-split', {
        detail: { primaryId: split.primaryId, secondaryId: split.secondaryId },
      }))
    })
  }, [loading, localWidgets])

  // Listen for global widget-update events (used by SecondaryPane editors)
  useEffect(() => {
    function handleWidgetUpdateEvent(e) {
      const { widgetId, updates } = e.detail || {}
      if (widgetId && updates && isLocalDev) {
        handleWidgetUpdate(widgetId, updates)
      }
    }
    document.addEventListener('storyboard:widget-update', handleWidgetUpdateEvent)
    return () => document.removeEventListener('storyboard:widget-update', handleWidgetUpdateEvent)
  }, [handleWidgetUpdate, isLocalDev])
  ...
  // Synchronous (not useEffect) so child components read current state
  // during render — useEffect would leave stale data for the useMemo
  // in ChromeWrappedWidget that calls findConnectedSplitTarget().
  {
