...
export function getSplitPaneLabel(widget) {
  ...
  if (widget.type === 'agent') {
    const name = widget.props?.prettyName || widget.props?.agentId || '…'
    return `Agent · ${name}`
  }
  if (widget.type === 'prototype') {
...
// ── Split-screen URL persistence ──────────────────────────────────

const SPLIT_PARAM = 'split'

/**
 * Write split-screen state to the URL as `?split=leftId,rightId`.
 * Uses replaceState to avoid polluting browser history.
 * @param {string} primaryId — the widget that initiated the expand
 * @param {string} secondaryId — the connected split target
 */
export function writeSplitToURL(primaryId, secondaryId) {
  const url = new URL(window.location.href)
  url.searchParams.set(SPLIT_PARAM, `${primaryId},${secondaryId}`)
  window.history.replaceState({}, '', url.toString())
}

/**
 * Remove split-screen state from the URL.
 */
export function clearSplitFromURL() {
  const url = new URL(window.location.href)
  if (!url.searchParams.has(SPLIT_PARAM)) return
  url.searchParams.delete(SPLIT_PARAM)
  window.history.replaceState({}, '', url.toString())
}

/**
 * Read split-screen widget IDs from the URL.
 * @returns {{ primaryId: string, secondaryId: string } | null}
 */
export function readSplitFromURL() {
  const params = new URLSearchParams(window.location.search)
  const val = params.get(SPLIT_PARAM)
  if (!val) return null
  const [primaryId, secondaryId] = val.split(',')
