/**
 * Story A — Inline Toolbar Swap crop interaction.
 *
 * When the user activates crop, the existing toolbar buttons fade out and are
 * replaced in-place by Save, Undo, Cancel and live dimensions. On
 * save/cancel the original toolbar fades back in. No extra panels or floating
 * bars — the toolbar shell stays in the same position throughout.
 */
import { useState, useRef, useCallback, useEffect } from 'react'
import styles from './image-crop-inline.story.module.css'
...
export function InlineToolbarCrop() {
  ...
  return (
    <div className={styles.cropStoryContainer}>
      {/* Single toolbar shell — contents swap between default and crop states */}
      <div className={styles.toolbar}>
        {/* Default toolbar row */}
        <div className={`${styles.toolbarRow} ${cropping ? styles.toolbarRowHidden : styles.toolbarRowVisible}`}>
          ...
          <button className={styles.toolbarBtn} onClick={handleStartCrop} title="Crop image">
            <CropIcon />
          </button>
          <button className={styles.toolbarBtn} title="Image actions"><ImageIcon /></button>
        </div>

        {/* Crop toolbar row — replaces default when cropping */}
        <div className={`${styles.toolbarRow} ${cropping ? styles.toolbarRowVisible : styles.toolbarRowHidden}`}>
          <button className={`${styles.toolbarAction} ${styles.toolbarActionSave}`} onClick={handleSave}>
            <CheckIcon /> Save
          </button>
          {savedCrop && (
            <button className={styles.toolbarAction} onClick={handleUndo}>
              <UndoIcon /> Undo
            </button>
          )}
          <button className={`${styles.toolbarAction} ${styles.toolbarActionCancel}`} onClick={handleCancel}>
            <XIcon /> Cancel
          </button>
      ...
      <div className={styles.description}>
        <h3>Story A — Inline Toolbar Swap</h3>
        <p>
          The existing widget toolbar is replaced in-place while crop mode is active.
          Buttons fade out and Save, Undo, Cancel fade in at the same position.
          On save or cancel the original toolbar buttons return. Minimal chrome —
          reuses the toolbar&apos;s exact position with no extra floating UI.
        </p>
      </div>
    </div>
  )
